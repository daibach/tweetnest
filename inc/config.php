<?php
	...
	$config = array(
		...
		'twitter_screenname' => getenv('APPSETTING_TWITTER_SCREENNAME'), // Twitter screen name of the one whose tweets are being recorded
		'your_tw_screenname' => getenv('APPSETTING_TWITTER_SCREENNAME'), // Your Twitter screen name -- the one we log in as. Usually the same as above, but does not have to be.
		...
		// Database values
		'db'                 => array(
			'hostname'       => getenv('APPSETTING_DB_HOSTNAME'), // The hostname of your database server. Usually 'localhost'
			'username'       => getenv('APPSETTING_DB_USERNAME'), // The username to log in to your database
			'password'       => getenv('APPSETTING_DB_PASSWORD'), // The password to log in to your database
			'database'       => getenv('APPSETTING_DB_DATABASE'), // The database name
			'table_prefix'   => getenv('APPSETTING_DB_TBPREFIX'), // The prefix to table names in the database
			'charset'	 => ''  // The charset of your database
		),

		// OPTIONAL -------------------------------------------
		// If you want to access your maintenance PHP files by HTTP, specify an admin password.
		'maintenance_http_password' => getenv('APPSETTING_MAINTENANCE_PASSWORD'),
